...
int main()
{
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		modelArc3 = glm::translate(modelArc3, glm::vec3(-5.6f, 0.3f, -2.0f));
		...
		modelArc4 = glm::translate(modelArc4, glm::vec3(-3.9f, 0.3f, -2.0f));
		...
		modelArc5 = glm::translate(modelArc5, glm::vec3(-2.3f, 0.3f, -2.0f));
...
void Animation() {
	...
	//Animacion para que el perrito camine
	if (dogAnim == 1) {
		if (!step) {       //Estado 1, en falso
			RLegs += 0.003f;
			FLegs += 0.003f;
			head += 0.003f;
			tail += 0.003f;
		...
		if (dogPos.z < limite) { //Se mueve el perrito si no ha llegado al limite
			dogPos.z += 0.0001f;
		}
		else {
			dogAnim = 0; // Se detiene antes del limite del piso
		}
	}
